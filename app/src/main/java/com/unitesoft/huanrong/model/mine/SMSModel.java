package com.unitesoft.huanrong.model.mine;
...
import com.unitesoft.huanrong.ApiService.MineService;
...
import com.unitesoft.huanrong.Bean.mine.SmsBean;
...
public class SMSModel {
     ...
     * 发送短信验证码
     */
    public void sendValidateCode(final String userPhone, final String source) {
        ...
        try {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    // 发送验证码
                    sendValidateMsg(userPhone,source);
                }
            }).start();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void sendValidateMsg(final String userPhone, final String source) {
        ...
        final Call<LoginBean> login = mineService.login(new LoginPostBean(userPhone, ""));
        login.enqueue(new Callback<LoginBean>() {
            ...
            public void onResponse(Call<LoginBean> call, Response<LoginBean> response) {
                ...
                if ("斗金APP注册".equals(source)) {
                    if (102 == businesscode) {
                        yanzhengma(userPhone,source);
                    } else {
                        onSMSListener.failed("用户已存在");
                    }
                }else {
                    if (102==businesscode){
                        onSMSListener.failed("用户不存在，需要注册");
                    }else {
                        yanzhengma(userPhone,source);
                    }
                }
    ...
    private void yanzhengma(String userPhone, final String source){
        Retrofit retrofit2 = CommonUtil.retrofit(ConstanceValue.smsurl);
        MineService mineService2 = retrofit2.create(MineService.class);
        Call<ResponseBody> responseBodyCall = mineService2.sendSMS(userPhone, "doujin", source);
        responseBodyCall.enqueue(new Callback<ResponseBody>() {
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                try {
                    ...
                    String success = smsBean.getSuccess();
                    if (TextUtils.equals("1", success)) {
                        onSMSListener.failed("短信发送失败,错误代码" + smsBean.getFlag());
                    }else {
                        String validateNum = smsBean.getValidateNum();
                        onSMSListener.checkValidateSuccess(validateNum);
                        onSMSListener.failed("短信发送成功");
                    }
                } catch (IOException e) {
                    e.printStackTrace();
                }
     ...
     * @param password
     */
    public void useRegister(final String userPhone, String validateCode, String sendValidateCode,final String password) {
        if (TextUtils.isEmpty(userPhone)) {
            onSMSListener.failed("手机号码不能为空");
            return;
        }
     ...
     * @param password
     */
    public void checkValidate(final String userPhone, String validateCode, String sendValidateCode, final String password){
        if (TextUtils.isEmpty(userPhone)) {
            onSMSListener.failed("手机号码不能为空");
            return;
        }
